...
def monitor_interactions():
    ...
    try:
        ...
        for event in events:
            if event['type'] == 'click':
                element_info = (f"Elemento {event['tag']} com ID='{event['id']}', classe='{event['class']}', "
                                f"texto='{event['text']}'")
                capture_interaction("Clique", element_info)
            elif event['type'] == 'keypress':
                element_info = (f"Tecla '{event['key']}' pressionada no elemento {event['tag']} com ID='{event['id']}', "
                                f"classe='{event['class']}', texto='{event['text']}'")
...
log_area.insert(tk.END, "Clique em Inciar Gravação...\n")
log_area.see(tk.END)
